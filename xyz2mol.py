...
def charge_is_OK(
    BO, AC, charge, DU, atomic_valence_electrons, atoms, valences, allow_charged_fragments=True
):
    # total charge
    Q = 0
...
def BO_is_OK(
    ...
    check_charge = charge_is_OK(
        BO, AC, charge, DU, atomic_valence_electrons, atoms, valences, allow_charged_fragments
    )
...
def BO2mol(
    mol, BO_matrix, atoms, atomic_valence_electrons, mol_charge, allow_charged_fragments=True
):
    """
    ...
    bo_shape = len(BO_matrix)
    num_atoms = len(atoms)
    BO_valences = list(BO_matrix.sum(axis=1))

    if bo_shape != num_atoms:
        raise RuntimeError(
            "sizes of adjMat ({0:d}) and Atoms {1:d} differ".format(bo_shape, num_atoms)
        )
    ...
    for i in range(bo_shape):
        for j in range(i + 1, bo_shape):
...
def main():
    ...
    parser.add_argument(
        "--no-graph", action="store_true", help="Run xyz2mol without networkx dependencies"
    )
    ...
    parser.add_argument(
        "--use-huckel", action="store_true", help="Use Huckel method for atom connectivity"
    )
    parser.add_argument(
        "-o",
        "--output-format",
